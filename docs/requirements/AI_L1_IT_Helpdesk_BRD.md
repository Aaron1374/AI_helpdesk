...
**Business Requirements Document | Capstone Group Project**
...
1. Automate safe first-line triage and basic troubleshooting.
1. Create consistently categorized and prioritized tickets.
1. Detect when a problem should be escalated to a human or another support tier.
1. Provide L1 engineers with complete context when taking over.
1. Measure AI resolution, escalation and support workload.
...
1. Employee B reports: "The HR portal is not working." The AI checks the application status and discovers a simulated outage; it creates/escalates a ticket with evidence.
1. Employee C reports: "VPN stopped working after I changed my network." The AI asks clarifying questions, runs safe checks and escalates if it cannot confidently resolve the issue.
1. Show the L1 engineer view and demonstrate takeover using the conversation and diagnostic context.
1. Submit a prompt-injection attempt requesting another employee’s ticket data and show that authorization boundaries remain intact.
1. Introduce a controlled service/tool failure and use observability information to diagnose it.
1. End by showing ticket analytics and the AI-resolution/escalation metrics.
